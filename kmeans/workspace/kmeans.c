...
int main(int argc, const char **argv) {
  ...
  assert(fin);
  uint32_t samples_size = atoi(argv[3]);
  uint32_t features_size = 1792;
  ...
  FILE *pFile = fopen("label.txt", "w");
  FILE *pFile1 = fopen("centroids.txt", "w");
  for(i=0; i<samples_size; i++){
    fprintf(pFile, "%d %d\n", i, assignments[i]);
  }
  for(i=0; i<clusters_size; i++){
    for(j=0; j<features_size;j++){
      fprintf(pFile1, "%f ", centroids[i * features_size + j]);
    }
    fprintf(pFile1, "\n");
  }
  fclose(pFile);
